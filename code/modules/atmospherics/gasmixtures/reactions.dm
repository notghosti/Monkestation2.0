...
// On TG, this code was deleted 6 years ago and replaced with the more convenient HFR.
// This is the reaction Plasmic Fusion. It triggers as Tritium, CO2, and Plasma are present.
// Spaghetti code has been cleaned up. Now Plasmic Fusion has its own variables.
...
/datum/gas_reaction/fusion/react(datum/gas_mixture/air, datum/holder)
	...
	var/toroidal_size = (2*PI)+TORADIANS(arctan((air.volume-PLASMIC_FUSION_TOROID_VOLUME_BREAKEVEN)/PLASMIC_FUSION_TOROID_VOLUME_BREAKEVEN)) //The size of the phase space hypertorus
	var/gas_power = 0
	var/list/cached_fusion_power = GAS_META[META_GAS_FUSION_POWER]
	for (var/gas_id in cached_moles)
		gas_power += (cached_fusion_power[gas_id] * cached_moles[gas_id])
	var/instability = MODULUS((gas_power*PLASMIC_FUSION_INSTABILITY_GAS_POWER_FACTOR)**2,toroidal_size) //Instability effects how chaotic the behavior of the reaction is
	cached_scan_results[id] = instability//used for analyzer feedback

	var/plasma = (plasma_moles - PLASMIC_FUSION_MOLE_THRESHOLD)/(scale_factor) //We have to scale the amounts of carbon and plasma down a significant amount in order to show the chaotic dynamics we want
	var/carbon = (carbon_dioxide_moles - PLASMIC_FUSION_MOLE_THRESHOLD)/(scale_factor) //We also subtract out the threshold amount to make it harder for fusion to burn itself out.
	...
	cached_moles[/datum/gas/carbon_dioxide] = carbon*scale_factor + PLASMIC_FUSION_MOLE_THRESHOLD
	var/delta_plasma = plasma_moles - cached_moles[/datum/gas/plasma]

	reaction_energy += delta_plasma*PLASMIC_FUSION_PLASMA_BINDING_ENERGY //Energy is gained or lost corresponding to the creation or destruction of mass.
	...
	if(air.thermal_energy() + reaction_energy < 0) //No using energy that doesn't exist.
		cached_moles[/datum/gas/plasma] = plasma_moles
		cached_moles[/datum/gas/carbon_dioxide] = carbon_dioxide_moles
		return NO_REACTION
	...
	if(reaction_energy > 0)
		...
		cached_moles[/datum/gas/oxygen] += PLASMIC_FUSION_TRITIUM_MOLES_USED*(reaction_energy*PLASMIC_FUSION_TRITIUM_CONVERSION_COEFFICIENT)
		cached_moles[/datum/gas/nitrous_oxide] += PLASMIC_FUSION_TRITIUM_MOLES_USED*(reaction_energy*PLASMIC_FUSION_TRITIUM_CONVERSION_COEFFICIENT)
	else
		air.assert_gases(/datum/gas/bz,/datum/gas/nitrium)
		cached_moles[/datum/gas/bz] += PLASMIC_FUSION_TRITIUM_MOLES_USED*(reaction_energy*-PLASMIC_FUSION_TRITIUM_CONVERSION_COEFFICIENT)
		cached_moles[/datum/gas/nitrium] += PLASMIC_FUSION_TRITIUM_MOLES_USED*(reaction_energy*-PLASMIC_FUSION_TRITIUM_CONVERSION_COEFFICIENT)
	...
	if(reaction_energy)
		if(location)
			...
			if(prob(PERCENT(particle_chance)))
				location.fire_nuclear_particle()
			var/rad_power = max((PLASMIC_FUSION_RAD_COEFFICIENT/instability) + 250, 15000) //Wow!
